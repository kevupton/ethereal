...
trait ResourceInterface
{
    ...
    private function execute(Request $request, $type, array $data = array())
    {
        try {
            if (!$this->hasErrors()) {
                ...
                if (method_exists($this, "before$type")) {
                    $this->{"before$type"}($request);
                }
                if ($this->isSuccess()) {
                    $callable = $type . 'Main';
                    ...
                    if ($this->isSuccess() &&
                        method_exists($this, "after$type")) {
                        $this->{"after$type"}($request);
                    }
                }
            }
    ...
    private function getMainClass()
    {
        if ($this->isRepository()) {
            return $this->getInstantiated()->getClass();
        } else {
    ...
    private function getErrorMessage($key, array $data = array())
    {
        ...
        if (array_key_exists($key, $this->trans_errors)) {
            ...
            if (array_key_exists($key, $this->error_messages)) {
                ...
                if (array_key_exists($key, $this->default_errors)) {
